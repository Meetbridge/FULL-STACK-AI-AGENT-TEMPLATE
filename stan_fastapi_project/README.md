...
```bash
# Local files
uv run stan_fastapi_project rag-ingest /path/to/docs/ --collection documents --recursive
# S3/MinIO
uv run stan_fastapi_project rag-sync-s3 --collection documents --prefix docs/
```

### Search

```bash
uv run stan_fastapi_project rag-search "your query" --collection documents
```

### Manage collections

```bash
uv run stan_fastapi_project rag-collections   # List all
uv run stan_fastapi_project rag-stats          # Show stats
uv run stan_fastapi_project rag-drop <name>    # Delete collection
```
...
```bash
uv run stan_fastapi_project cmd rag-sources          # List configured sources
uv run stan_fastapi_project cmd rag-source-add       # Add a new source
uv run stan_fastapi_project cmd rag-source-sync      # Trigger sync for a source
uv run stan_fastapi_project cmd rag-source-remove    # Remove a source
```
